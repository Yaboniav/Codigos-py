...
def validador(plantilla):
    ...
    :return: DataFrame corregido y DataFrame con índices y descripción del error o "Sin errores".
        """
#FASES
    # Cargamos el DataFrame con el "Cod Dane" desde la ruta especificada
    df_dane = pd.read_excel(r'D:/OneDrive - Grupo EPM/1. PLANEACIÓN DE INFRAESTRUCTURA/04_PIR/3_Ejecutado/BD_seguimiento/3. inputs/1. información basica - redes - trafos - municipios/cod - regionales.xlsx', engine='openpyxl')
    df_municipio = pd.read_excel(r'D:/OneDrive - Grupo EPM/1. PLANEACIÓN DE INFRAESTRUCTURA/04_PIR/3_Ejecutado/BD_seguimiento/3. inputs/1. información basica - redes - trafos - municipios/cod - municipios.xlsx', engine='openpyxl')
    df_UCs = pd.read_excel(r'D:/OneDrive - Grupo EPM/1. PLANEACIÓN DE INFRAESTRUCTURA/04_PIR/3_Ejecutado/BD_seguimiento/3. inputs/1. información basica - redes - trafos - municipios/UCs.xlsx', engine='openpyxl')
    subestaciones = pd.read_csv("D:/OneDrive - Grupo EPM/1. PLANEACIÓN DE INFRAESTRUCTURA/04_PIR/3_Ejecutado/BD_seguimiento/3. inputs/1. información basica - redes - trafos - municipios/subestaciones.csv", encoding="ISO-8859-1",sep=';',decimal='.', dtype={"IU": str})
    lineas = pd.read_excel("D:/OneDrive - Grupo EPM/1. PLANEACIÓN DE INFRAESTRUCTURA/04_PIR/3_Ejecutado/BD_seguimiento/3. inputs/1. información basica - redes - trafos - municipios/líneas.xlsx", engine='openpyxl', dtype={"IUS inicial": str, "IUS final": str}) 
    ...
    for idx, row in plantilla.iterrows():
        descripcion_error = []
        # poner conductor = 0 cuando no son "km de conductor"
        
        # Validar y corregir 'Año entrada operación'
        if pd.isna(row['Año entrada operación']) or not isinstance(row['Año entrada operación'], (int, float)) or row['Año entrada operación'] != datetime.datetime.now().year:
            descripcion_error.append("Error en 'Año entrada operación'")
            plantilla.at[idx, 'Año entrada operación'] = datetime.datetime.now().year
        ...
        if pd.isna(numero_conductores) or numero_conductores not in [0, 1, 2, 3, 4]:
            ...
            #Verifica si 'DESCRIPCION' contiene "km de conductor" y 'Número de conductores' es 0
        elif "km de conductor" in descripcion and numero_conductores == 0:
            descripcion_error.append(f"Error en fila {idx}: 'Número de conductores' debe ser 1, 2, 3 o 4 cuando es una UC asociada a conductores'")
        ...
        if duplicados_fid[idx]:
            descripcion_error.append("Error en 'Código FID': Valor duplicado o campo vacio.")
            
        # Validar que el campo "DESCRIPCION" no tenga valores vacíos
        descripcion = row['DESCRIPCION']
        if pd.isna(descripcion) or (isinstance(descripcion, str) and descripcion.strip() == ''):
        ...
        if tipo_inversion in ["I", "III"]:
            if pd.isna(codigo_uc_rep) or codigo_uc_rep.strip() == '':
                ...
                # Verifica si 'DESCRIPCION' contiene "km de conductor" y 'Número de conductores' es 0
                elif "km de conductor" in descripcion_rep and numero_conductores == 0:
                    descripcion_error.append(f"Error en fila {idx}: 'Número de conductores' debe ser 1, 2, 3 o 4 cuando es una UC asociada a conductores'")
        ...
        # validar que el campo concepto UPME no este vacio   
